            ...
            <div className={`p-2 rounded-md space-y-2 ${isFinalRound ? 'max-w-md' : 'max-w-sm'} w-full mx-auto`}>
               {scoresDefined ? (
                ...
                <div className="flex items-center justify-between gap-2 rounded-md bg-secondary/50 p-2 text-sm">
                    <span className="flex-1 text-right truncate">{match.team1 ? teamToKey(match.team1) : placeholder1}</span>
                    <span className="text-muted-foreground font-bold text-xs">vs</span>
                    <span className="flex-1 text-left truncate">{match.team2 ? teamToKey(match.team2) : placeholder2}</span>
                </div>
               )}
            </div>
        </div>
